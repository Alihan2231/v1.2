...
class SidebarItem(tk.Frame):
    ...
    def __init__(self, parent, icon, text, command=None, is_active=False, **kwargs):
        super().__init__(parent, bg=THEME["sidebar_background"], **kwargs)
        self.icon = icon
        self.text = text
        self.command = command
        ...
        # Etkileşimler için event'leri bağla
        self.bind("<Enter>", self._on_enter)
        self.bind("<Leave>", self._on_leave)
        self.bind("<Button-1>", self._on_press)
    ...
    def _create_widgets(self):
        # Simge etiketi
        self.icon_label = tk.Label(self, text=self.icon, font=("Arial", 14), 
                                  bg=THEME["sidebar_background"], padx=10, pady=10)
        
        # Metin etiketi
        self.text_label = tk.Label(self, text=self.text, font=("Arial", 11), 
                                  bg=THEME["sidebar_background"], padx=5, pady=10,
                                  anchor="w")
        
        # Aktif göstergesi
        self.indicator = tk.Frame(self, width=4, bg=THEME["sidebar_background"])
        self.indicator.pack(side="left", fill="y")
        
        # Simge ve metin yerleştir
    ...
    def _update_colors(self):
        ...
        if self.is_active:
            self.indicator.config(bg=THEME["primary"])
            text_color = THEME["primary"]
        else:
            self.indicator.config(bg=THEME["sidebar_background"])
            text_color = THEME["text_secondary"]
        
        self.icon_label.config(fg=text_color)
